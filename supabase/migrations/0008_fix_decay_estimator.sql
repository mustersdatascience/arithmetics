-- Correctie op de decay-schatter uit 0007
--
-- Die vergeleek elke meting met de vorige meting. Maar de vorige meting was
-- zelf ook al weggezakt sinds de keer dáárvoor, dus de verhouding tussen twee
-- opeenvolgende metingen zegt niets over de helling. De fout kwam aan het licht
-- bij het valideren tegen gesimuleerde data met een bekende waarde.
--
-- Correct is een regressie met eigen basislijn per som. Voor rel = b·(1 + g·x)
-- met x = ln(1 + dagen) geldt na deling door de mediaan van die som:
--
--     y = 1/(1 + g·x̄) + g/(1 + g·x̄) · x
--
-- Dus de helling gedeeld door het intercept levert g, ongeacht de gemiddelde
-- tussentijd in de data.
...
begin
  ...
  -- ---- decay: helling van de verhouding tot je eigen basislijn per som,
  --      afgezet tegen ln(1 + dagen sinds de vorige keer)
  with base as (
    select problem_key,
           percentile_cont(0.5) within group (order by rel) as b
    from _rel where ok
    group by problem_key having count(*) >= 3
  ),
  gaps as (
    select
      r.rel / b.b as y,
    ...
    from _rel r
    join base b on b.problem_key = r.problem_key
    where r.ok and b.b > 0
  ),
  usable as (
